...
def main(dataName_A,dataName_B,K_size):
    ...
    parser.add_argument('-dataName_A', action='store', dest='dataName_A', default=dataName_A)
    parser.add_argument('-dataName_B', action='store', dest='dataName_B', default=dataName_B)
    parser.add_argument('-negNum', action='store', dest='negNum', default=7, type=int)
    parser.add_argument('-userLayer', action='store', dest='userLayer', default=[K_size,2*K_size,4*K_size,8*K_size,4*K_size,2*K_size,K_size])
    parser.add_argument('-itemLayer', action='store', dest='itemLayer', default=[K_size,2*K_size,4*K_size,8*K_size,4*K_size,2*K_size,K_size])
    #parser.add_argument('-userLayer', action='store', dest='userLayer', default=[2*K_size,4*K_size,8*K_size,16*K_size,8*K_size,4*K_size,2*K_size])
    #parser.add_argument('-itemLayer', action='store', dest='itemLayer', default=[2*K_size,4*K_size,8*K_size,16*K_size,8*K_size,4*K_size,2*K_size])
...
class Model:
    def __init__(self, args):
        ...
        self.model_N2V_A=Word2Vec.load("Node2vec_"+self.dataName_A+"_KSize_"+str(self.KSize)+".model")
        self.model_N2V_B=Word2Vec.load("Node2vec_"+self.dataName_B+"_KSize_"+str(self.KSize)+".model")
    ...
    def add_embedding_matrix(self):
        self.user_item_embedding_A = tf.convert_to_tensor(self.dataSet_A.getEmbedding())
        self.item_user_embedding_A= tf.transpose(self.user_item_embedding_A)
        self.user_item_embedding_B = tf.convert_to_tensor(self.dataSet_B.getEmbedding())
        self.item_user_embedding_B= tf.transpose(self.user_item_embedding_B)

    def add_model(self):
        user_input_A = tf.nn.embedding_lookup(self.user_item_embedding_A, self.user_A)
        item_input_A= tf.nn.embedding_lookup(self.item_user_embedding_A, self.item_A)
        user_input_B = tf.nn.embedding_lookup(self.user_item_embedding_B, self.user_B)
        item_input_B= tf.nn.embedding_lookup(self.item_user_embedding_B, self.item_B)
        
        def init_variable(shape, name):
            return tf.Variable(tf.truncated_normal(shape=shape, dtype=tf.float32, stddev=0.25), name=name)
        ...
        with tf.name_scope("User_Layer"):
            ...
            user_W1_A = init_variable([self.KSize, self.userLayer[0]], "user_W1_A")
            user_out_A = tf.matmul(user_input_A, user_W1_A)
            user_input_B = tf.nn.embedding_lookup(node_features_B, self.user_B)
            user_W1_B = init_variable([self.KSize, self.userLayer[0]], "user_W1_B")
            user_out_B = tf.matmul(user_input_B, user_W1_B)
            
            # Element-wise Attention for common users
            user_W_Attention_A_A=tf.Variable(tf.truncated_normal(shape=[self.shape_A[1], self.userLayer[0]], mean=0.5, dtype=tf.float32, stddev=0.25), name="user_W_Attention_A_A") # the weights of Domain A for Domain A
            user_W_Attention_B_A=1-user_W_Attention_A_A # the weights of Domain B for Domain A
            user_W_Attention_A_A_lookup=tf.nn.embedding_lookup(user_W_Attention_A_A, self.user_A)
            user_W_Attention_B_A_lookup=tf.nn.embedding_lookup(user_W_Attention_B_A, self.user_B)
            user_out_A_Combined = tf.add(tf.multiply(user_out_A, user_W_Attention_A_A_lookup),tf.multiply(user_out_B, user_W_Attention_B_A_lookup))
            user_W_Attention_B_B=tf.Variable(tf.truncated_normal(shape=[self.shape_B[1], self.userLayer[0]], mean=0.5, dtype=tf.float32, stddev=0.25), name="user_W_Attention_B_B") # the weights of Domain B for Domain B
            user_W_Attention_A_B=1-user_W_Attention_B_B # the weights of Domain A for Domain B
            user_W_Attention_B_B_lookup=tf.nn.embedding_lookup(user_W_Attention_B_B, self.user_B)
            user_W_Attention_A_B_lookup=tf.nn.embedding_lookup(user_W_Attention_A_B, self.user_A)
            user_out_B_Combined = tf.add(tf.multiply(user_out_A, user_W_Attention_A_B_lookup),tf.multiply(user_out_B, user_W_Attention_B_B_lookup))
            user_out_A=user_out_A_Combined
            user_out_B=user_out_B_Combined
            
            # full-connected layers (MLP)
            for i in range(0, len(self.userLayer)-1):
                W_A = init_variable([self.userLayer[i], self.userLayer[i+1]], "user_W_A"+str(i+2))
                b_A= init_variable([self.userLayer[i+1]], "user_b_A"+str(i+2))
                user_out_A = tf.nn.relu(tf.add(tf.matmul(user_out_A, W_A), b_A))
                W_B = init_variable([self.userLayer[i], self.userLayer[i+1]], "user_W_B"+str(i+2))
                b_B= init_variable([self.userLayer[i+1]], "user_b_B"+str(i+2))
                user_out_B = tf.nn.relu(tf.add(tf.matmul(user_out_B, W_B), b_B))

        with tf.name_scope("Item_Layer"):
            # Choose the node2vec embeddings as the input
            node_features_A=self.model_N2V_A.wv.vectors
            node_features_B=self.model_N2V_B.wv.vectors
            item_input_A= tf.nn.embedding_lookup(node_features_A, self.shape_A[0]+self.item_A)
            item_W1_A = init_variable([self.KSize, self.itemLayer[0]], "item_W1_A")
            item_out_A = tf.matmul(item_input_A, item_W1_A)
            item_input_B= tf.nn.embedding_lookup(node_features_B, self.shape_B[0]+self.item_B)
            item_W1_B = init_variable([self.KSize, self.itemLayer[0]], "item_W1_B")
            item_out_B = tf.matmul(item_input_B, item_W1_B)
            # full-connected layers (MLP)
            for i in range(0, len(self.itemLayer)-1):
                W_A = init_variable([self.itemLayer[i], self.itemLayer[i+1]], "item_W_A"+str(i+2))
                b_A= init_variable([self.itemLayer[i+1]], "item_b_A"+str(i+2))
                item_out_A = tf.nn.relu(tf.add(tf.matmul(item_out_A, W_A), b_A))
                W_B = init_variable([self.itemLayer[i], self.itemLayer[i+1]], "item_W_B"+str(i+2))
                b_B= init_variable([self.itemLayer[i+1]], "item_b_B"+str(i+2))
                item_out_B = tf.nn.relu(tf.add(tf.matmul(item_out_B, W_B), b_B))
                
        norm_user_output_A = tf.sqrt(tf.reduce_sum(tf.square(user_out_A), axis=1))
        norm_item_output_A = tf.sqrt(tf.reduce_sum(tf.square(item_out_A), axis=1))
        norm_user_output_B = tf.sqrt(tf.reduce_sum(tf.square(user_out_B), axis=1))
        norm_item_output_B = tf.sqrt(tf.reduce_sum(tf.square(item_out_B), axis=1))
        self.y_A = tf.reduce_sum(tf.multiply(user_out_A, item_out_A), axis=1, keepdims=False) / (norm_item_output_A* norm_user_output_A)
        self.y_A= tf.maximum(1e-6, self.y_A)
        self.y_B = tf.reduce_sum(tf.multiply(user_out_B, item_out_B), axis=1, keepdims=False) / (norm_item_output_B* norm_user_output_B)
        self.y_B= tf.maximum(1e-6, self.y_B)

    def add_loss(self):
        regRate_A = self.rate_A / self.maxRate_A
        losses_A= regRate_A * tf.log(self.y_A) + (1 - regRate_A) * tf.log(1 - self.y_A)
        loss_A = -tf.reduce_sum(losses_A)
        self.loss_A = loss_A
        
        regRate_B = self.rate_B / self.maxRate_B
        losses_B= regRate_B * tf.log(self.y_B) + (1 - regRate_B) * tf.log(1 - self.y_B)
    ...
    def run(self):
        ...
        for epoch in range(self.maxEpochs):
            ...
            print("Epoch ", epoch, "Domain A: {} TopK: {} HR: {}, NDCG: {}".format(self.dataName_A,topK,hr_A, NDCG_A))
            print("Epoch ", epoch, "Domain B: {} TopK: {} HR: {}, NDCG: {}".format(self.dataName_B,topK,hr_B, NDCG_B))
            if hr_A > best_hr_A:
                best_hr_A = hr_A
                best_epoch_A = epoch
        ...
        print("Domain A: Best hr: {}, NDCG: {}, At Epoch {}; Domain B: Best hr: {}, NDCG: {}, At Epoch {}".format(best_hr_A, best_NDCG_A, best_epoch_A, best_hr_B, best_NDCG_B, best_epoch_B))
        ...
        matname='GA-DTCDR_'+str(self.dataName_A)+'_'+str(self.dataName_B)+'_KSize_'+str(self.KSize)+'_Result.mat'
        scio.savemat(matname,{'allResults_A':allResults_A,'allResults_B':allResults_B,'bestPerformance':bestPerformance})
        print("Training complete!")

    def run_epoch(self, sess, verbose=10):
        train_u_A, train_i_A, train_r_A= self.dataSet_A.getInstances(self.train_A, self.negNum)
        ...
        train_u_B, train_i_B, train_r_B= self.dataSet_B.getInstances(self.train_B, self.negNum)
